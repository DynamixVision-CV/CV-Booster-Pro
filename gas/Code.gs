...
const ADMIN_PASSWORD = "DynamixVision2026@";

const SHEET_CODES = "Codes";
const SHEET_SUBMISSIONS = "Soumissions";
const SHEET_SCAN_CODES = "CodesScan";
const SHEET_SCAN_RESULTS = "ResultatsScan";
...
function doPost(e) {
  try {
    ...
    switch (action) {
      ...
      // ---- Module Scan ATS ----
      case "verifyScanCode":
        return jsonResponse(verifyScanCode(body.code));
      case "consumeScanCode":
        return jsonResponse(consumeScanCode(body.code));
      case "saveScanResult":
        return jsonResponse(saveScanResult(body));
      case "adminScanStats":
        return jsonResponse(adminScanStats(body.password));
      case "adminListScanResults":
        return jsonResponse(adminListScanResults(body.password));
      case "adminGetScanResult":
        return jsonResponse(adminGetScanResult(body.password, body.scanId));
      case "adminUnlockScanCode":
        return jsonResponse(adminUnlockScanCode(body.password, body.code));
...
/* ============================================================
   MODULE SCAN ATS — GESTION DES CODES SCAN
   ============================================================
   Fonctionne exactement comme les codes CV (Codes / Soumissions),
   mais sur des feuilles séparées, avec un préfixe "SCAN-" au lieu
   de "CVB-", pour ne jamais mélanger les deux usages.
   ============================================================ */

/**
 * Vérifie si un code Scan existe et est encore disponible.
 * Ne le consomme PAS (la consommation se fait après analyse réussie,
 * dans consumeScanCode, appelée depuis le frontend).
 */
function verifyScanCode(code) {
  const sheet = getSheet(SHEET_SCAN_CODES);
  const data = sheet.getDataRange().getValues();
  const target = normalizeCode(code);
  ...
  for (let i = 1; i < data.length; i++) {
    ...
    if (rowCode === target) {
      const statut = data[i][1];
      if (statut === "Utilisé") {
        return { ok: false, error: "Ce code Scan a déjà été utilisé. Contactez l'administrateur si besoin." };
      }
      return { ok: true, valid: true };
    }
  }
  return { ok: false, error: "Code Scan invalide. Vérifiez votre saisie." };
}

/**
 * Marque un code Scan comme "Utilisé" de façon définitive.
 * Appelé uniquement après qu'une analyse a été menée à son terme.
 */
function consumeScanCode(code) {
  const sheet = getSheet(SHEET_SCAN_CODES);
  const data = sheet.getDataRange().getValues();
  const target = normalizeCode(code);
  ...
  for (let i = 1; i < data.length; i++) {
    ...
    if (rowCode === target) {
      if (data[i][1] === "Utilisé") {
        return { ok: false, error: "Ce code Scan a déjà été utilisé." };
      }
      sheet.getRange(i + 1, 2).setValue("Utilisé");
      sheet.getRange(i + 1, 3).setValue(new Date());
      return { ok: true };
  ...
  return { ok: false, error: "Code Scan introuvable." };
}

/* ============================================================
   MODULE SCAN ATS — RÉSULTATS DE SCAN
   ============================================================ */

/**
 * Enregistre le résultat d'un scan pour suivi côté admin.
 * Le détail complet (scores, conseils) est stocké en JSON,
 * comme pour les soumissions de CV.
 */
function saveScanResult(body) {
  const sheet = getSheet(SHEET_SCAN_RESULTS);
  const now = new Date();
  const code = normalizeCode(body.code);
  const resultatJSON = JSON.stringify(body.resultat || {});

  const scanId = "SCN-" + Utilities.getUuid().substring(0, 8).toUpperCase();
  sheet.appendRow([
    scanId,
    code,
    body.fileName || "",
    body.globalScore !== undefined ? body.globalScore : "",
    now,
    resultatJSON,
    "Terminé"
  ]);
...
function rowToScanObject(row) {
  let resultat = {};
  try { resultat = JSON.parse(row[5] || "{}"); } catch (e) { resultat = {}; }
  return {
    scanId: row[0],
    code: row[1],
    fileName: row[2],
...
/* ============================================================
   MODULE SCAN ATS — ADMINISTRATION
   ============================================================ */

function adminScanStats(password) {
  if (!checkAdmin(password)) return { ok: false, error: "Accès refusé." };
  const codesSheet = getSheet(SHEET_SCAN_CODES);
  const codesData = codesSheet.getDataRange().getValues();
  let total = 0, utilises = 0, disponibles = 0;
  for (let i = 1; i < codesData.length; i++) {
    total++;
  ...
  const resultsSheet = getSheet(SHEET_SCAN_RESULTS);
  const nbScans = resultsSheet.getLastRow() - 1;
  return { ok: true, stats: { total, utilises, disponibles, nbScans: Math.max(0, nbScans) } };
}

function adminListScanResults(password) {
  if (!checkAdmin(password)) return { ok: false, error: "Accès refusé." };
  const sheet = getSheet(SHEET_SCAN_RESULTS);
  const data = sheet.getDataRange().getValues();
  const list = [];
  for (let i = 1; i < data.length; i++) {
    const obj = rowToScanObject(data[i]);
    delete obj.resultat; // pas besoin du détail complet dans la liste
    list.push(obj);
  }
  return { ok: true, scans: list.reverse() };
}

function adminGetScanResult(password, scanId) {
  if (!checkAdmin(password)) return { ok: false, error: "Accès refusé." };
  const sheet = getSheet(SHEET_SCAN_RESULTS);
  const data = sheet.getDataRange().getValues();
  for (let i = 1; i < data.length; i++) {
    if (data[i][0] === scanId) {
      return { ok: true, scan: rowToScanObject(data[i]) };
    }
  }
  return { ok: false, error: "Résultat de scan introuvable." };
}

/**
 * Permet à l'admin de réactiver un code Scan marqué "Utilisé",
 * par exemple si le client a rencontré un problème technique
 * et doit refaire son analyse.
 */
function adminUnlockScanCode(password, code) {
  if (!checkAdmin(password)) return { ok: false, error: "Accès refusé." };
  const sheet = getSheet(SHEET_SCAN_CODES);
  const data = sheet.getDataRange().getValues();
  const target = normalizeCode(code);
...
function initialiserFeuilles() {
  ...
  let scanCodesSheet = ss.getSheetByName(SHEET_SCAN_CODES);
  if (!scanCodesSheet) {
    scanCodesSheet = ss.insertSheet(SHEET_SCAN_CODES);
    scanCodesSheet.appendRow(["Code", "Statut", "DateUtilisation"]);
    scanCodesSheet.setFrozenRows(1);
  }

  let scanResultsSheet = ss.getSheetByName(SHEET_SCAN_RESULTS);
  if (!scanResultsSheet) {
    scanResultsSheet = ss.insertSheet(SHEET_SCAN_RESULTS);
    scanResultsSheet.appendRow(["ScanId", "Code", "FileName", "GlobalScore", "DateCreation", "ResultatJSON", "Statut"]);
    scanResultsSheet.setFrozenRows(1);
  }
...
/**
 * Génère un lot de codes uniques aléatoires pour le module Scan ATS,
 * au format "SCAN-XXXX-XXXX", et les ajoute à la feuille "CodesScan"
 * avec le statut "Disponible". À exécuter depuis l'éditeur Apps Script.
 * Modifie NB_CODES_SCAN si tu veux en générer un autre nombre.
 */
function genererCodesScan() {
  const NB_CODES_SCAN = 500;
  const sheet = getSheet(SHEET_SCAN_CODES);
  ...
  const rows = [];
  let generated = 0;
  while (generated < NB_CODES_SCAN) {
    const code = "SCAN-" + randomCodeSegment(4) + "-" + randomCodeSegment(4);
    if (existing.has(code)) continue;
    existing.add(code);
    rows.push([code, "Disponible", ""]);
    generated++;
  ...
  SpreadsheetApp.getUi().alert(generated + " codes Scan générés avec succès dans la feuille 'CodesScan'.");
}
