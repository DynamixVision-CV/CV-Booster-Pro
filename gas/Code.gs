 ...
 * Ne le consomme PAS — utilisé pour valider à l'écran avant de
 ...
 * de son code, même si le code est marqué "Utilisé" (cas de blocage).
 */
function getSubmissionByCode(code) {
  ...
  for (let i = 1; i < data.length; i++) {
    if (normalizeCode(data[i][1]) === target) {
      return {
        ok: true,
        submission: rowToSubmissionObject(data[i])
      };
    }
  }
  return { ok: false, error: "Aucune donnée trouvée pour ce code." };
}
