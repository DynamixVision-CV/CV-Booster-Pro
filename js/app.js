...
const App = (() => {
  ...
  const state = {
    ...
    paletteId: null,
    autosaveTimer: null,
    cameFromAdmin: false // true quand on consulte le détail d'un client depuis l'espace admin
  };
  ...
  /* ============================================================
     ÉTAPE 1 — SÉLECTION DU PROFIL (point d'entrée depuis l'accueil)
     ============================================================ */
  ...
  /* ============================================================
     ÉTAPE 2 — SÉLECTION DE LA PALETTE / DESIGN (avec aperçu visuel)
     ============================================================ */

  // Chemin des images d'aperçu, une par palette (mini-CV illustratif avec données factices)
  const PALETTE_PREVIEW_PATH = "assets/previews/preview_";
  ...
  function renderPaletteCards() {
    ...
    container.innerHTML = CV_CONFIG.palettes.map(p => `
      <div class="palette-card palette-card-visual" data-palette="${p.id}">
        <div class="palette-preview-img-wrap">
          <img src="${PALETTE_PREVIEW_PATH}${p.id}.png" alt="Aperçu design ${p.name}" loading="lazy">
        </div>
        <h4>${p.name}</h4>
        <p>${p.description}</p>
      </div>
    ...
    container.querySelectorAll(".palette-card").forEach(card => {
      card.addEventListener("click", () => {
        ...
        // Le choix visuel est fait : on demande maintenant le code d'accès pour continuer.
        setTimeout(() => showView("viewGate"), 200);
      });
    });
  }

  /* ============================================================
     ÉTAPE 3 — ÉCRAN DE CODE (demandé après le choix profil + design)
     ============================================================ */
  ...
  function initGateView() {
    ...
    async function submitCode() {
      ...
      // — y compris son profil et son design déjà choisis à l'époque, qui
      // remplacent le choix tout juste fait à l'écran précédent si différent.
      if (res.submission) {
      ...
      // Nouveau parcours : le profil et le design ont déjà été choisis
      // juste avant (state.profileId / state.paletteId) -> on démarre le formulaire.
      if (!state.profileId || !state.paletteId) {
        // Filet de sécurité : si l'utilisateur arrive ici sans être passé par
        // le choix profil/design (lien direct, retour arrière navigateur...),
        // on le renvoie au tout début du parcours de création.
        errorEl.textContent = "";
        toast("Merci de choisir d'abord votre profil et votre design.", "error");
  ...
  function initFormNav() {
    ...
    document.getElementById("formNextBtn").addEventListener("click", async () => {
      ...
      if (FormEngine.isLastSection()) {
        ...
        document.getElementById("backToAdminFromPreviewBtn").style.display = "none";
  ...
  function initPreviewActions() {
    ...
    document.getElementById("backToAdminFromPreviewBtn").addEventListener("click", () => {
  ...
  function initAdmin() {
    ...
    if (loginBtn) {
      loginBtn.addEventListener("click", async () => {
        ...
        showView("viewAdminDashboard");
        loadAdminData();
        document.dispatchEvent(new CustomEvent("admin-login-success", { detail: { password: pwd } }));
      });
    }
    ...
    const backHomeBtn = document.getElementById("adminBackToHomeBtn");
    if (backHomeBtn) {
  ...
  async function openAdminSubmissionDetail(submissionId) {
    ...
    document.getElementById("backToAdminFromPreviewBtn").style.display = "block";
  ...
  return { init, renderProfileCards };
})();
