...
const App = (() => {
  ...
  function initGateView() {
    ...
    const recoverBtn = document.getElementById("gateRecoverBtn");

    btn.addEventListener("click", () => submitCode());
    input.addEventListener("keydown", (e) => { if (e.key === "Enter") submitCode(); });
    recoverBtn.addEventListener("click", () => recoverByCode());
    ...
    async function recoverByCode() {
      ...
      errorEl.textContent = "";
      if (!code) {
        errorEl.textContent = "Saisissez d'abord votre code pour récupérer votre CV.";
        return;
      }
      showLoader(true);
      const res = await Api.getSubmissionByCode(code);
      showLoader(false);

      if (!res.ok) {
        errorEl.textContent = res.error || "Aucune donnée trouvée pour ce code.";
        return;
      }
      ...
      toast("Vos données ont été retrouvées, vous reprenez où vous en étiez.", "success");
      showView("viewForm");
      renderFormSection();
    }
  }
  ...
  function initFormNav() {
    ...
    document.getElementById("formNextBtn").addEventListener("click", async () => {
      ...
      if (!FormEngine.validateCurrentSection(container)) {
        toast("Merci de remplir tous les champs obligatoires (*).", "error");
        return;
      }
