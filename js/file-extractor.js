...
const FileExtractor = (() => {
  ...
  const TESSERACT_CDN_URL = "https://cdnjs.cloudflare.com/ajax/libs/tesseract.js/5.0.2/tesseract.min.js";
  let tesseractLoadPromise = null;

  /**
   * Charge Tesseract.js uniquement au moment où on en a réellement besoin
   * (première image ou premier PDF scanné à analyser), plutôt qu'au
   * chargement de la page pour tout le monde : cette librairie est lourde
   * et n'était utile qu'à une minorité d'utilisateurs — la charger
   * systématiquement ralentissait le démarrage de toute l'application.
   */
  function loadTesseract() {
    ...
    tesseractLoadPromise = new Promise((resolve, reject) => {
      ...
      script.onerror = () => {
        ...
        reject(new Error("Impossible de charger le moteur de reconnaissance de texte (OCR). Vérifiez votre connexion internet et réessayez."));
      };
  ...
  async function ocrPdfPages(pdf) {
    ...
    try {
      ...
      for (let pageNum = 1; pageNum <= pageCount; pageNum++) {
        ...
        const viewport = page.getViewport({ scale: 1.5 }); // suffisant pour l'OCR, plus léger qu'une pleine résolution d'impression sur mobile
