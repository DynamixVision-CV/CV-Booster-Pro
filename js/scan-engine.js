...
const ScanEngine = (() => {
  ...
  const OFFER_MATCH_GOOD    = 0.55;  // bonne adéquation : peut postuler avec confiance
  const OFFER_MATCH_PARTIAL = 0.35;  // adéquation partielle : possible avec réserves
  // sous OFFER_MATCH_PARTIAL : faible adéquation, on déconseille honnêtement
  ...
  function scoreKeywords(cvText, jobText) {
    ...
    // "Pas d'offre" = aucun texte réel fourni (collé ou extrait d'un fichier).
    // On se base sur le compte de mots BRUT (countWords), pas sur les mots-clés
    // filtrés : filtrer les mots vides / acronymes de 2 lettres avant de décider
    // si une offre existe faisait passer à tort de vraies offres courtes (ou
    // riches en sigles comme UX, IT, PM, QA) pour "aucune offre fournie".
    const trimmedJob = (jobText || "").trim();
    if (!trimmedJob || countWords(trimmedJob) < 8) {
    ...
    if (keywords.length < 3) {
      // Une offre a bien été fournie, mais trop courte/vague pour en tirer des
      // mots-clés fiables (ex : juste un intitulé de poste sans description).
      findings.tips.push("L'offre fournie est trop courte pour une analyse fiable des mots-clés : collez si possible le texte intégral de l'annonce (missions, compétences requises, profil recherché).");
      return { score: 12, max: 25, findings, matched: [], missing: [], noJob: true };
    }
