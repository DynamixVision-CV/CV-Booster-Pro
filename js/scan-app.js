/* ============================================================
   CV BOOSTER PRO — Module "Scan ATS" (scan-app.js) — Version 2
   ============================================================
   Orchestre le parcours du Scan : Code -> Upload -> Analyse -> Résultats.
   Les suggestions de métiers ont été supprimées (non fiables sans IA).
   Le verdict d'adéquation CV/offre est amélioré et plus précis.
   ============================================================ */
...
const ScanApp = (() => {
  ...
  function initScanGate() {
    ...
    async function submitScanCode() {
      ...
      errorEl.textContent = "";
      if (!code) { errorEl.textContent = "Merci de saisir votre code Scan."; return; }

      showLoader(true);
      const res = await Api.verifyScanCode(code);
      showLoader(false);

      if (!res.ok) { errorEl.textContent = res.error || "Code invalide."; return; }
  ...
  function initScanUploadView() {
    initDropzone("scanCvDropzone", "scanCvInput", "scanCvStatus", async (file) => {
      ...
      try {
        ...
        if (result.extractionWarning) {
          setFileStatus("scanCvStatus", `⚠️ "${file.name}" importé — ${result.extractionWarning.toLowerCase()}`, "warn");
        } else {
    ...
    document.getElementById("scanBackBtn").addEventListener("click", () => showView("viewScanGate"));
    document.getElementById("scanAnalyzeBtn").addEventListener("click", runAnalysis);
  }

  async function runAnalysis() {
    if (!state.cvFile?.text) {
      toast("Merci d'importer votre CV avant de lancer l'analyse.", "error");
      return;
    }

    const jobText = state.jobInputMode === "paste"
      ? document.getElementById("scanJobTextarea").value.trim()
      : (state.jobText || "");
    ...
    try {
      const cvMeta = {
        ...
        hasComplexTables: state.cvFile.meta?.hasComplexTables
      };
      ...
      document.getElementById("backToAdminFromScanBtn").style.display = "none";
  ...
  function simpleMarkdownToHtml(str) {
    const escaped = escapeHtmlScan(str)
  ...
  function renderCriterionCard(c) {
    ...
    return `
      <div class="scan-criterion-card">
        ...
        ${f.good.length ? `<div class="scan-finding-group scan-finding-good">
          ${f.good.map(t => `<div class="scan-finding-item">✅ ${escapeHtmlScan(t)}</div>`).join("")}
        </div>` : ""}
        ${f.bad.length ? `<div class="scan-finding-group scan-finding-bad">
          ${f.bad.map(t => `<div class="scan-finding-item">⚠️ ${escapeHtmlScan(t)}</div>`).join("")}
        </div>` : ""}
        ${f.tips.length ? `<div class="scan-finding-group scan-finding-tip">
          ${f.tips.map(t => `<div class="scan-finding-item">💡 ${escapeHtmlScan(t)}</div>`).join("")}
        </div>` : ""}
      </div>`;
  }

  function renderOfferFitVerdict(v) {
    if (!v) return "";
    return `
      <div class="scan-section-block">
        <h3 class="scan-section-title">🤝 Adéquation avec l'offre fournie</h3>
        <div class="scan-content-card scan-fit-${v.level}">
          <div class="scan-fit-header">
            <span class="scan-fit-title">${escapeHtmlScan(v.title)}</span>
            <span class="scan-fit-ratio">${v.ratio}% de correspondance</span>
          </div>
          <p class="scan-fit-message">${escapeHtmlScan(v.message)}</p>
          ${v.missedTechnicalCount > 0 ? `<p class="scan-fit-technical-warn">🔧 ${v.missedTechnicalCount} compétence(s) technique(s) clé(s) de l'offre absente(s) de votre CV.</p>` : ""}
          <div class="scan-fit-advice">${simpleMarkdownToHtml(v.advice)}</div>
        </div>
      </div>`;
  }
    ...
    container.innerHTML = `
      <div class="scan-global-card scan-verdict-${result.verdictClass}">
        ...
        <div class="scan-global-text">
          ...
          <p>Analyse ATS : lisibilité, mots-clés, structure, orientation résultats et présentation — basée sur le guide CV Booster Pro.</p>
        </div>
      </div>
      ...
      ${renderOfferFitVerdict(result.offerFitVerdict)}
    `;
  }
  ...
  async function openAdminScanDetail(scanId) {
    ...
    document.getElementById("backToAdminFromScanBtn").style.display = "inline-block";
    showView("viewScanResults");
  ...
  document.addEventListener("DOMContentLoaded", () => {
    ...
    const backToAdminBtn = document.getElementById("backToAdminFromScanBtn");
    if (backToAdminBtn) {
      backToAdminBtn.addEventListener("click", () => {
        showView("viewAdminDashboard");
        document.querySelectorAll("#adminTabs .scan-tab").forEach(b => b.classList.remove("active"));
        document.querySelector("#adminTabs .scan-tab[data-admin-tab='scan']").classList.add("active");
        document.getElementById("adminPanelCv").classList.remove("active");
        document.getElementById("adminPanelScan").classList.add("active");
        loadAdminScanData();
      });
    }
  });
