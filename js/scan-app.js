...
const ScanApp = (() => {
  ...
  function initScanUploadView() {
    ...
    initDropzone("scanJobDropzone", "scanJobInput", "scanJobStatus", async (file) => {
      setFileStatus("scanJobStatus", "Lecture du fichier en cours… (la reconnaissance de texte sur une image peut prendre quelques secondes)", "loading");
      try {
        const result = await FileExtractor.extract(file);
        state.jobText = result.text;
        state.jobFileMeta = { fileName: result.fileName, fileType: result.fileType };
        if (result.extractionWarning) {
          setFileStatus("scanJobStatus", `⚠️ "${file.name}" importé, mais ${result.extractionWarning.toLowerCase()}`, "warn");
        } else {
          setFileStatus("scanJobStatus", `✅ "${file.name}" importé avec succès.`, "ok");
        }
      } catch (err) {
        state.jobFileMeta = null;
        state.jobText = ""; // on ne garde pas un texte d'offre périmé d'un import précédent
  ...
  async function runAnalysis() {
    ...
    // On lit les deux sources et on prend celle qui a du contenu
    // (évite le cas où le client colle du texte mais le mode actif est "file" ou vice-versa)
    const jobTextFromPaste = document.getElementById("scanJobTextarea").value.trim();
    const jobTextFromFile  = state.jobText || "";
    let jobText = jobTextFromPaste || jobTextFromFile;
