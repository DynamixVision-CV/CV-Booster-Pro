...
const ScanApp = (() => {
  ...
  // Convertit le **gras** markdown léger utilisé dans scan-engine.js, et les retours à la ligne, en HTML sûr.
  ...
  function renderJobSuggestions(jobSuggestions) {
    if (!jobSuggestions) return "";
    return `
      <div class="scan-section-block">
        <h3 class="scan-section-title">🧭 À quels postes pouvez-vous postuler avec ce CV ?</h3>
        <p class="scan-section-desc">Cette analyse porte sur le contenu réel de votre CV (compétences et expériences détectées) — indépendamment de la mise en forme.</p>
        <div class="scan-content-card">
          ${simpleMarkdownToHtml(jobSuggestions.summary)}
        </div>
      </div>
    `;
  }

  function renderOfferFitVerdict(offerFitVerdict) {
    if (!offerFitVerdict) return "";
    return `
      <div class="scan-section-block">
        <h3 class="scan-section-title">🤝 Adéquation avec l'offre fournie</h3>
        <div class="scan-content-card scan-fit-${offerFitVerdict.level}">
          <div class="scan-fit-ratio">${offerFitVerdict.ratio}% de correspondance (${offerFitVerdict.matchedCount}/${offerFitVerdict.total} éléments attendus retrouvés)</div>
          ${simpleMarkdownToHtml(offerFitVerdict.message)}
        </div>
      </div>
    `;
  }
  ...
  function renderResults(result) {
    ...
    container.innerHTML = `
      ...
      ${renderJobSuggestions(result.jobSuggestions)}
    `;
  }
  ...
  document.addEventListener("DOMContentLoaded", () => {
    ...
    if (backToAdminBtn) {
      backToAdminBtn.addEventListener("click", () => {
        ...
        // S'assure que l'onglet "Scan ATS" est actif et rechargé au retour
