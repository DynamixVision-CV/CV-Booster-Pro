...
/**
 * Transforme un texte multiligne en liste HTML à puces simples (•).
 * Chaque ligne non vide devient un élément de liste.
 * Les lignes déjà préfixées par "-", "•" ou "*" sont nettoyées de leur préfixe.
 */
function textToList(str) {
  ...
  return `<ul class="cv-bullet-list">${lines.map(l => `<li>${escapeHtml(l)}</li>`).join("")}</ul>`;
}
    ...
    case "competences": {
      ...
      return `
        <div class="cv-section">
          ...
          <div class="cv-section-line"></div>
          ${c1 ? `<div class="cv-competences-bloc">
            <p class="cv-competences-label">Compétences</p>
            ${textToList(c1)}
          </div>` : ""}
          ${c2 ? `<div class="cv-competences-bloc">
            <p class="cv-competences-label">Certifications</p>
            ${textToList(c2)}
          </div>` : ""}
        </div>`;
    }
          ...
          <ul class="cv-list">
            ${items.map(it => `
              <li class="cv-item">
                ...
                ${textToList(it.TACHES_EFFECTUEES)}
              </li>
            `).join("")}
          </ul>
        </div>`;
          ...
          <ul class="cv-list">
            ${items.map(it => `
              <li class="cv-item">
                ...
                ${textToList(it.ACTIVITES_MENEES)}
              </li>
            `).join("")}
          </ul>
        </div>`;
          ...
          <ul class="cv-list cv-list-langues">
            ${items.map(it => `
              <li class="cv-lang-item">
                ...
                <span class="cv-lang-score">${Number(it.NIVEAU) || 0}<span class="cv-lang-max">/5</span></span>
              </li>
            `).join("")}
          </ul>
        </div>`;
    ...
    ">
      ...
      <footer class="cv-footer">
        ...
        <div class="cv-signature">
          <img src="${data.SIGNATURE_DATA}" alt="Signature" class="cv-signature-img">
        </div>` : ""}
      </footer>
    </div>
  `;
